...
async def insert_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if name and name not in queue:
        ...
        print(f"{get_time()}|{chat.title if chat.title else chat.username}|{queue_name}: insert {name} ({position + 1})", flush=True)
...
async def remove_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if position is not None and 0 <= position < len(queue):
        ...
        print(f"{get_time()}|{chat.title if chat.title else chat.username}|{queue_name}: remove {removed_name} ({position + 1})", flush=True)
        await queue_manager.send_queue_message(update, context, queue_name)
    elif name and name in queue:  # Или по имени
        position = queue.index(name)
        queue.remove(name)
        print(f"{get_time()}|{chat.title if chat.title else chat.username}|{queue_name}: remove {name} ({position + 1})", flush=True)
        await queue_manager.send_queue_message(update, context, queue_name)
...
async def replace_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"{get_time()}|{chat.title if chat.title else chat.username}|{queue_name}: replace {name1} ({pos1 + 1}) with {name2} ({pos2 + 1})", flush=True)
