...
class TestAdminCommands:
    ...
    def setup_common(self, admin_mocks):
        ...
        self.context.args = []
        self.context.bot_data = {"queue_service": admin_mocks["queue_service"]}
    ...
    async def test_delete_queue_success(self, admin_mocks):
        ...
        # Мок поиска очереди
        admin_mocks["queue_service"].repo.get_queue_by_name.return_value = Queue(id="q1", name="My Queue")
        admin_mocks["queue_service"].repo.get_queue_message_id.return_value = 999
        ...
        admin_mocks["queue_service"].delete_queue.assert_awaited_once_with(self.context, self.ctx)

    async def test_delete_queue_not_found(self, admin_mocks):
        self.context.args = ["Unknown"]
        admin_mocks["queue_service"].repo.get_queue_by_name.return_value = None

        await admin_module.delete_queue(self.update, self.context, ctx=self.ctx)

        admin_mocks["delete_later"].assert_awaited_once()
        assert "не найдена" in admin_mocks["delete_later"].call_args[0][2]
        admin_mocks["queue_service"].delete_queue.assert_not_awaited()
    ...
    async def test_delete_all_queues(self, admin_mocks):
        admin_mocks["queue_service"].repo.get_list_message_id.return_value = 100
        admin_mocks["queue_service"].repo.get_all_queues.return_value = {
        ...
        admin_mocks["queue_service"].repo.clear_list_message_id.assert_awaited_once()

        # Удаление очередей (2 раза)
        assert admin_mocks["queue_service"].delete_queue.await_count == 2
    ...
    async def test_insert_user_success(self, admin_mocks):
        ...
        admin_mocks["queue_service"].insert_into_queue.assert_awaited_once_with(self.ctx, "User", 1)
        admin_mocks["queue_service"].update_queue_message.assert_awaited_once()
    ...
    async def test_remove_user_success(self, admin_mocks):
        ...
        admin_mocks["queue_service"].remove_from_queue.return_value = ("User", 1)

        await admin_module.remove_user(self.update, self.context, ctx=self.ctx)

        admin_mocks["queue_service"].remove_from_queue.assert_awaited_once_with(self.ctx, pos=None, user_name="User")
        admin_mocks["queue_service"].update_queue_message.assert_awaited_once()
    ...
    async def test_remove_user_not_found_in_queue(self, admin_mocks):
        ...
        admin_mocks["queue_service"].remove_from_queue.return_value = (None, None)

        await admin_module.remove_user(self.update, self.context, ctx=self.ctx)

        admin_mocks["queue_service"].update_queue_message.assert_not_awaited()
        admin_mocks["delete_later"].assert_awaited_once()
    ...
    async def test_replace_users(self, admin_mocks):
        ...
        admin_mocks["queue_service"].replace_users_queue.assert_awaited_once_with(self.ctx, 1, 2, "u1", "u2")
        admin_mocks["queue_service"].update_queue_message.assert_awaited_once()
    ...
    async def test_rename_queue_success(self, admin_mocks):
        ...
        admin_mocks["queue_service"].repo.get_all_queues.return_value = {"q1": Queue(id="q1", name="Old")}
        ...
        admin_mocks["queue_service"].rename_queue.assert_awaited_once_with(self.ctx, "New")
        admin_mocks["queue_service"].update_queue_message.assert_awaited_once()
    ...
    async def test_rename_queue_duplicate(self, admin_mocks):
        ...
        admin_mocks["queue_service"].rename_queue.assert_not_awaited()
    ...
    async def test_set_expire_time_success(self, admin_mocks):
        ...
        admin_mocks["queue_service"].repo.get_queue_by_name.return_value = Queue(id="q1", name="Queue")

        await admin_module.set_queue_expiration_time(self.update, self.context, ctx=self.ctx)

        # 5 часов * 3600
        admin_mocks["queue_service"].auto_cleanup_service.reschedule_expiration.assert_awaited_once_with(
            self.ctx, 18000
        )
        admin_mocks["delete_later"].assert_awaited_once()
    ...
    async def test_set_description_success(self, admin_mocks):
        ...
        admin_mocks["queue_service"].set_queue_description.assert_awaited_once_with(self.ctx, "Cool Desc")
        admin_mocks["queue_service"].update_queue_message.assert_awaited_once()
    ...
    async def test_set_description_reset(self, admin_mocks):
        ...
        admin_mocks["queue_service"].set_queue_description.assert_awaited_once_with(self.ctx, None)
