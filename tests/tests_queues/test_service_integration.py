"""
Дополнительные тесты для facade_service.py с фокусом на недостающие пути кода
"""
...
class TestQueueFacadeServiceIntegration:
    """Интеграционные тесты для facade_service.py - фокус на граничные случаи"""

    # ========== Тесты для методов вывода информации ==========
    async def test_get_count_queues(self, mock_repo, facade_service):
        ...
        result = await facade_service.get_count_queues(123)
        assert result == 2

    async def test_get_count_queues_empty(self, mock_repo, facade_service):
    ...
    async def test_generate_queue_name(self, mock_repo, facade_service):
    ...
    async def test_generate_queue_name_empty_chats(self, mock_repo, facade_service):
    ...
    async def test_create_queue_success(self, mock_repo, facade_service):
        ...
        result = await facade_service.create_queue(mock_context, ctx, 3600)

        assert result == "new_q_id"
        assert ctx.queue_id == "new_q_id"

    # ========== Тесты для delete_queue ==========
    async def test_delete_queue_success(self, mock_repo, facade_service):
        ...
        await facade_service.delete_queue(mock_context, ctx)

        mock_repo.delete_queue.assert_called_once_with(123, "q1")

    async def test_delete_queue_error(self, mock_repo, mock_logger, facade_service):
    ...
    async def test_join_to_queue_success(self, mock_repo, facade_service):
        """Успешное присоединение к очереди"""

        ctx = ActionContext(chat_id=123, chat_title="Test", queue_id="q1", actor="user")
        user = User(id=456, is_bot=False, first_name="TestUser")

        facade_service.user_service.get_user_display_name = AsyncMock(return_value="Display Name")
        mock_repo.add_to_queue = AsyncMock(return_value=1)

        result = await facade_service.join_to_queue(ctx, user)

        assert result == 1

    async def test_join_to_queue_already_exists(self, mock_repo, facade_service):
    ...
    async def test_leave_from_queue_success(self, mock_repo, facade_service):
        ...
        result = await facade_service.leave_from_queue(ctx, user)

        assert result == 1

    async def test_leave_from_queue_not_in_queue(self, mock_repo, facade_service):
